-- Trajetos (2+ paradas) em reservas_transfer + RPC da frota.
-- Atribuição ida/volta usa motorista_id por linha (já existente); não altera RLS.

ALTER TABLE public.reservas_transfer
  ADD COLUMN IF NOT EXISTS trajetos jsonb NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE public.reservas_transfer
  DROP CONSTRAINT IF EXISTS reservas_transfer_trajetos_is_array;

ALTER TABLE public.reservas_transfer
  ADD CONSTRAINT reservas_transfer_trajetos_is_array
  CHECK (jsonb_typeof(trajetos) = 'array');

COMMENT ON COLUMN public.reservas_transfer.trajetos IS
  'Paradas do tipo multiplos_trajetos: [{embarque, desembarque, data, hora, passageiros}]. Outros tipos usam [].';

CREATE INDEX IF NOT EXISTS reservas_transfer_user_id_created_at_idx
  ON public.reservas_transfer (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS reservas_transfer_motorista_id_idx
  ON public.reservas_transfer (motorista_id)
  WHERE motorista_id IS NOT NULL AND btrim(motorista_id) <> '';
